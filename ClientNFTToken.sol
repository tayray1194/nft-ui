...
contract ClientNFTToken is ERC721, ERC721Enumerable, ERC721URIStorage, Pausable, Ownable, ERC721Burnable {
    using Counters for Counters.Counter;

    Counters.Counter private _tokenIdCounter;
    uint256 public maxSupply = 10000;
    uint256 public price = 10000000000000000; // price of each NFT in FTM...set to 0.01 for testing purposes
    bool public saleOpen = false;
    bool public presaleOpen = false;
    string public baseURI;

    address withdrawWallet1 = 0x43FeD4E94Ed856327B765961F07E4C96eaE11b70; // withdraw wallets set to tayray's metamask test wallets for testing purposes
    address withdrawWallet2 = 0xFcEDC5c0c1adDfD21128DAE73266fDbe7a9167B7;

    mapping(address => uint256) public Whitelist;

    receive() external payable {}

    constructor() ERC721('Fox NFT', 'FOX') {}
    ...
    function whitelistAddress(address[] memory who, uint256 amount) public onlyOwner { // add addresses to make them eligible to participate in presale
        for (uint256 i = 0; i < who.length; i++) Whitelist[who[i]] = amount;
    }

    function withdraw() public onlyOwner { // only callable by owner. Allows FTM in NFT contract to be withdrawed to specified wallet addresses
        uint256 balance = address(this).balance;
        payable(withdrawWallet1).transfer((balance * 600) / 1000);
        payable(withdrawWallet2).transfer((balance * 400) / 1000);
    }

    function pause() public onlyOwner { // I could implement a pausable contract if needed
        _pause();
    }
    ...
    function toggleSale() public onlyOwner { // allows owner to open/close the sale
        saleOpen = !saleOpen;
    }

    function togglePresale() public onlyOwner { // allows owner to open/close the presale
    ...
    function setBaseURI(string memory newBaseURI) public onlyOwner { // each minted token's tokenURI will be a string consisting of the "baseURI"+"tokenId"
        baseURI = newBaseURI;
    }
    ...
    function mint(uint256 amount) public payable { // if sale is open then anyone can call this function to mint token's...if presale is open then only whitelisted addresses can call this
        ...
        require(amount <= 3, 'only 3 per transaction allowed'); // limit on token mint amount per transaction
        for (uint256 i = 0; i < amount; i++) internalMint(msg.sender);
    }
